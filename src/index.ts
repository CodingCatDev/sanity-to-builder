import { createBuilderPostNotionPage } from './config';
...
// export const addFrameworkLanguageToNotion = async ({
//   groqStatement,
//   databaseId,
// }: {
//   groqStatement: string;
//   databaseId: string;
// }) => {
//   console.log(groqStatement);
//   const modelData = await sanity.fetch(groq`${groqStatement}`);

//   for (const d of modelData) {
//     console.log('Adding Model: ', d._type);
//     const res = createFrameworkLanguageNotionPage({
//       databaseId,
...
(async () => {
  ...
  // addFrameworkLanguageToNotion({
  //   groqStatement: createGroqSelectAllTypes(['framework']),
  //   databaseId: process.env.NOTION_FRAMEWORKS as string,
  // });

  // addFrameworkLanguageToNotion({
  //   groqStatement: createGroqSelectAllTypes(['language']),
  //   databaseId: process.env.NOTION_LANGUAGES as string,
  // });

  await createBuilderPostNotionPage({
    model: 'lesson',
    databaseId: process.env.NOTION_LESSONS,
  });
})();
